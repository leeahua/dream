...
public class TagsApiController {
    ...
    @PostMapping("/list")
    @LoginAnnotation
    public ResultVO list(@RequestBody TagsDto tagsDto){
        log.info("获取标签列表,参数信息：{}",tagsDto);
        if(tagsDto.getUserId()==null){
            return ResultVOUtil.failed(ResultEnum.PARAM_ERROR);
        }
        return ResultVOUtil.success(tagsService.findByStatus(CategoryStatusEnum.ACTIVITY.getCode(),tagsDto));
    }
